...
3. **Load the `dotnet-cop` skill before reviewing.** You MUST load `skills/dotnet-cop/SKILL.md`, then `skills/dotnet-cop/enforcement.md` (always), then the file-type sub-pages for each changed file. Skipping enforcement.md is a contract violation. **For schema/role/RLS/provisioning changes, consult the `db-provisioning` skill.**
...
- `scope` (optional): comma list among `minimal-api,isolation,ports-adapters,ef-core,csharp,ddd` (default all); `ddd` auto-enables when the diff touches domain files (`**/*.Domain/**`, `**/Domain/**`, `**/Core/**`) — pass `ddd` to force it on.
...
### 4. Load skill
You MUST load `skills/dotnet-cop/SKILL.md` (or `~/.claude/skills/dotnet-cop/SKILL.md` for Claude), then `skills/dotnet-cop/enforcement.md` (always — required before any findings are recorded). Then, for each file in the diff, load only the relevant sub-pages:

| File pattern | Sub-pages |
|---|---|
| `*Module.cs`, `*Extensions.cs` | modular-isolation |
| `*Endpoint.cs` (Minimal API handler) | minimal-api |
| `*Domain/**`, `*Core/**` (domain layer) | ddd (load `dotnet-ddd` review-checklist) |
...
**Tooling gap note:** If the repo lacks `.editorconfig` / Roslyn analyzer rules covering the BLOCK checks from `enforcement.md`, add a finding: `🟡 [MEDIUM] Recommend adopting enforcement-tooling.md analyzer config to enforce BLOCK rules automatically at CI.`
