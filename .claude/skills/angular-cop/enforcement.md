...
| Relative-import climbing instead of the project's tsconfig path alias | Grep `from '\.\./\.\.` (≥2 levels up) in changed files under the Angular app root (e.g. `src/app/**`) · ESLint `no-restricted-imports` pattern (see [[angular-cop-enforcement-tooling]]) | Check `compilerOptions.paths` in the project's tsconfig and use its aliases for cross-directory imports (e.g. gc.platform defines exactly one alias: `"@/*": ["./src/app/*"]` in `frontend/tsconfig.json`, mirrored in `frontend/vitest.config.ts`). Deep relative imports are hard to read and break on file moves. BAD: `import { Company } from '../../../core/models'` → GOOD: `import { Company } from '@/modules/companies/core/models'`. Same-directory `./sibling` imports and targets the alias cannot express (outside the aliased root, e.g. `src/environments/`) stay relative. Full rule: `angular-clean-architecture` → `layer-templates.md` §Import Conventions. |
