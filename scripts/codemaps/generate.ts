...
function classifyFiles(allFiles: string[]): Record<string, AreaInfo> {
  ...
  for (const area of Object.values(areas)) {
    ...
    area.entryPoints = area.files
      .filter(
        (f) =>
          /index\.(ts|tsx|js|jsx)$/.test(f) || /main\.(ts|tsx|js|jsx)$/.test(f),
      )
      .slice(0, 10);
  }
...
function generateAreaDoc(
  areaKey: string,
  area: AreaInfo,
  allFiles: string[],
): string {
  const fileCount = area.files.length;
  const totalLines = area.files.reduce(
    (sum, f) => sum + lineCount(path.join(ROOT, f)),
    0,
  );

  const entrySection =
    area.entryPoints.length > 0
  ...
  const fileSection = area.files
    .slice(0, 30)
    .map((f) => `| \`${f}\` | ${lineCount(path.join(ROOT, f))} |`)
    .join("\n");

  const moreFiles =
    area.files.length > 30
...
function generateIndex(
  ...
  const areaRows = Object.entries(areas)
    .map(
      ([key, area]) =>
        `| [${area.name}](./${key}.md) | ${area.files.length} files | ${
          area.directories
            .slice(0, 3)
            .map((d) => `\`${d}\``)
            .join(", ") || "—"
...
function main(): void {
  ...
  for (const [key, area] of Object.entries(areas)) {
    ...
    console.log(
      `[generate.ts] Written: ${rel(outPath)} (${area.files.length} files)`,
    );
  }
